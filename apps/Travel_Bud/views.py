...
def index(request):
    return render(request, 'Travel_Bud/index.html')
...
def register(request):
    ...
    if newuser[0] == False:
        for each in newuser[1]:
            messages.error(request, each) # Show message for each error
        return redirect('/')
    if newuser[0] == True:
...
def travel(request):
    ...
    return render(request, 'Travel_Bud/travelplan.html', context)
...
def addplan(request):
    if 'id' not in request.session:
        ...
        return render(request, 'Travel_Bud/addplan.html', context)
...
def show(request, travel_id):
    ...
    return render(request, 'Travel_Bud/showdetail.html', context)
